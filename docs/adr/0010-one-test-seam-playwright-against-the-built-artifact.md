...
- **Set a viewport ≥ 51rem.** Below it the page is in Reading Mode, where the Asides are `display: none` on the paper: the test would measure a column that is not there. This trap applies to **every** paper measurement in the suite, including the two-column heading alignment.
- **Assert `> 0`, not a floor.**

The tightest columns as of the last remeasure are IT Sheet 2 Main (+50.0) and EN Sheet 1 Aside (+51.8) — the canaries, useful for judging whether a content edit is close to the edge.
...
- **The PDF text assertion inflates content streams rather than taking a PDF text reader.** Chromium writes a `ToUnicode` CMap for every font it embeds, including the Type3 ones, so `tests/support/pdf.ts` walks the content stream directly instead of adding a 15MB dependency for one assertion. Two traps if it ever needs touching: **Type0 fonts address glyphs with two-byte codes and Type3 with one** — getting that wrong makes half the characters come out blank — and Chromium positions one glyph at a time, so the extracted run has no usable word spacing and both sides of a comparison have their whitespace stripped.
- **"Fonts embedded" is asserted as embedding, not by name.** Garet and Now are the unnamed Type3 faces (ADR-0009), so they cannot be checked by name. The suite asserts that no font relies on a face the reader must supply, and that every *named* font is one of the CV's. That leaves a hole — a CV reset entirely in Lato would satisfy it — which is closed by additionally requiring **at least one Type3 font per file**. It is the only handle those two faces offer.
...
- **The Drawer's toggle has no accessible name in Paper Mode**, because it is `display: none` above 51rem and therefore absent from the accessibility tree entirely. That is correct — four controls on paper, five in Reading Mode (`CONTEXT.md`) — but an accessible-name assertion written at the default viewport fails against it. Name assertions live in the Reading Mode block; the Paper Mode block asserts the toggle is *absent*.
