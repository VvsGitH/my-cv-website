...
// One PDF per Locale, captured from the built page (ADR-0001). Run from the
// repo root, after `astro build`. The whys behind the capture recipe below are
// in docs/issues/cv-website/08-pdf-render.md.
...
// Must stay the name Chrome.astro points the download control at.
const pdfPath = (locale) => `${outDir}/Vito_Paparella_Santorsola_CV_${locale.toUpperCase()}.pdf`;

const routeFor = (locale) =>
...
try {
  ...
  for (const locale of config.i18n.locales) {
    ...
    // Ticket 08, "The portrait was missing from the first PDF of every run".
