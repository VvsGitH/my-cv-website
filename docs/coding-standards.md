...
- **Theming:** `color-scheme: light dark` on `:root`, tokens as custom properties in `oklch()`, `light-dark()` for per-property pairs (keep a `prefers-color-scheme` fallback since it's only newly available). `@property` only if you animate a custom property.
  ...
  - **Put the whole theme ladder inside `@media screen`.** It is what keeps the PDF and the OG card out of the dark theme, for no duplicated literal: under `print` the ladder is inert and the base values — the light ones — remain. Same reasoning as the Reading Mode queries above.
