...
- `--color-muted` is gone. It was a grey that only ever answered to the cream; with the panel themed there was nothing left for it to answer to, and its two consumers — the proficiency-bar fill and the focus ring — read `--color-text`. `--color-signature` also lost its special case: it is read in both Modes now, rather than only in Reading Mode where the cream used to be absent.
...
- The proficiency bars have no track colour of their own: the track is `--color-text` at 20% over whatever is under the bar, so a bar's two halves cannot drift apart. 20% is the ratio, not a taste — 30% left only 3.08:1 back when the fill was the muted grey, and the sampled `#b1c0e1` it replaced failed 1.4.11 outright at 2.60:1. Measured on the built site now that the fill is the body ink, fill against its own track: **6.25:1** light / **5.46:1** dark on the panel, **5.56:1** / **4.46:1** on the page in Reading Mode.
...
- Fonts, **as asked**: **Garet** (Heavy → name/headings, Book → lighter display), **Now**, **Lato** (body), **Primera Signature** (script, signature only), sourced from `docs/assets/fonts/`. **As shipped:** the CV sets its own type instead — **JetBrains Mono** display, **Atkinson Hyperlegible** body, Primera Signature unchanged — and the raw sources are committed under `src/assets/fonts/`, with the subsets cut at build time (ADR-0020, ADR-0023). Garet was the one commercial face in the stack; matching the reference's weights is no longer a goal.
- Colored backgrounds (the Aside's panel, photo disc, proficiency bars) must survive into the PDF: `printBackground: true` + `-webkit-print-color-adjust: exact` / `print-color-adjust: exact`.
...
- Fonts — originally provided in the git-ignored `docs/assets/fonts/`. **No longer an owner-provided dependency:** every face is committed under `src/assets/fonts/`, both text families under the SIL OFL (ADR-0020, ADR-0023). The signature script face is the one carry-over, and the one with no licence on record.
