...
**Capture below 53.5rem and two fonts vanish.** Below the Reading Mode boundary the page lays out with the Aside's Blocks `display: none` on the paper and the Drawer closed. Chrome requests a font only when rendering needs one, so **Atkinson Hyperlegible Italic** (the Languages proficiency labels) and **Primera Signature** are never fetched. The print pass then needs them, and the PDF comes out wrong, with no error anywhere. A default 800×600 headless window reproduces it every time.
...
- **No face reaches the PDF as Type3 any more.** Garet and Now did — Chromium redraws CFF outlines as glyph procedures with no `BaseFont`, so nothing scanning `/BaseFont` could see them. JetBrains Mono and Atkinson are TrueType (ADR-0020), so every face is named and countable, and the workaround ADR-0010 built around that blind spot is retired. If a CFF face is ever added back, the blind spot returns with it.
