...
- Fonts — originally provided in the git-ignored `docs/assets/fonts/`. **No longer an owner-provided dependency:** every face is committed under `src/assets/raw-fonts/`, both text families under the SIL OFL (ADR-0020). The signature script face is the one carry-over, and the one with no licence on record.
