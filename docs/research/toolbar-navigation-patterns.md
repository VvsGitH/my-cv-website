...
Scope: the Toolbar of `src/components/chrome/Toolbar.tsx` / `toolbar.css` — a fixed vertical strip of five icon-only controls (drawer toggle below 48rem, language, PDF download, share, theme) that floats on the leading edge in Paper Mode and drops to the bottom-left in Reading Mode. Background: `CONTEXT.md`, [ADR-0006](../adr/0006-post-ticket-06-responsive-adjustments.md), [ADR-0007](../adr/0007-custom-modal-drawer-and-two-chrome-islands.md).
...
**Reading Mode (`width < 48rem`), at a 16 px root:**
...
1. **Drawer toggle** — one 44 px control, fixed at the **top of the leading edge**, `--space-m` from both edges. Present only below 48rem, as today. Sources: HIG "show or hide a sidebar appear at the far leading edge"; M3 "The menu icon and FAB should always be top-aligned"; M3 "Expanded navigation rails can open from menu buttons on mobile."
...
If the reveal is hover-triggered, [WCAG 1.4.13](https://www.w3.org/WAI/WCAG22/Understanding/content-on-hover-or-focus.html) applies: Dismissible, Hoverable, Persistent. A width-expanding rail whose labels are *inside* the control (not floating over content) sidesteps most of 1.4.13, because nothing is obscured and nothing needs dismissing — that is the cheaper design. **But an expanding rail changes the geometry the Toolbar floats over**, and at the medium tier `toolbar.css` already notes "an unscaled A4 Sheet already fills the viewport (ADR-0006) and there is no margin left to float over." So: expand toward the viewport edge, or gate the expansion on `width >= 101rem` where there is margin to spare.
...
1. **Drawer toggle → top of the leading edge**, below 48rem. This is the fix for pain point 1 and it is the best-sourced change in the whole note (HIG's "far leading edge" for the sidebar control; M3's "The menu icon and FAB should always be top-aligned").
