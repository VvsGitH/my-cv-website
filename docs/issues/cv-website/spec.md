...
| `--color-muted` | proficiency-bar fill, focus ring — a real grey only on the two cream surfaces; everywhere else it is `--color-text` |
...
- Four of those tokens are themed and carry a `-light` / `-dark` pair behind them (`--color-page-bg`, `--color-main-bg`, `--color-heading`, `--color-text`); the pairs are raw material and no component names one. `--color-muted` has no pair of its own: a third rung between the body ink and the surface cleared the contrast minimum on the paper but not on the page behind it, and Reading Mode reads on the page — so outside the cream the token is simply `--color-text`, and the Aside's panel and the Toolbar pin it back locally, both to `--color-cream-muted` — the grey that answers to the cream (the Toolbar names that token rather than `--color-aside-muted`, which rejoins the theme in Reading Mode while the cluster stays cream). Two are not themed at all — `--color-accent` and `--color-aside-bg` — because the cream Aside is the same surface in both themes and takes the light inks back locally (ADR-0015). `--color-signature` is the in-between case: it carries a pair like the themed five, but the pair is only consulted in Reading Mode, where the cream is gone; on paper it stays the blue in both themes like the rest of the Aside's inks. Its dark end is `--color-accent`, not a ramp step — a blue that deep has nowhere to go on a dark page.
...
- The proficiency bars have no track colour of their own: the track is `--color-muted` at 20% over whatever is under the bar, so a bar's two halves cannot drift apart. 20% is the ratio, not a taste — on the Aside's cream it holds the fill at ~3.5:1 against the track, where 30% left only 3.08:1 and the sampled `#b1c0e1` it replaced failed 1.4.11 outright at 2.60:1. In Reading Mode the fill is the body ink on the page instead, and the same 20% gives more: 5.6:1 in light, 4.5:1 in dark.
